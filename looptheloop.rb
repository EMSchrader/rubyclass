...
You need to  have the user make an array of all the things the user has. This will have to be in a loop. Start by asking them what they have, one by one, then use the operator << to add it to your usergear array. like this userGear << gearTheySaid
=end

until done
  puts "tell me one piece of gear you have. write done if you are done"
  userinput=gets.chomp
  usergear << userinput
  puts "You have #{usergear}."
  puts "Are you done? true of false?"
  input=gets.chomp
  if input=="true"
    done=true
  end
...
#gearNeeded = ["skis" , "boots", "poles"]
for gear in gearNeeded
  for things-you-have in userGear
    if gear = things-you-have
      puts you have it #{it}
end 
end
